...
export function countryDial(iso: string) {
  const row = countries.find((c) => c.code === iso);
  return (row?.phone || '').replace(/\D/g, '');
}

/** Map stored dial (e.g. 880) back to ISO code for selects. */
export function isoFromDial(dial: string) {
  const raw = String(dial || '').replace(/\D/g, '');
  if (!raw) return 'BD';
  const hit = countries.find((c) => c.phone.replace(/\D/g, '') === raw || c.code === dial);
  return hit?.code || 'BD';
}
