  ...
  try {
    ...
    if (search) {
      ...
      filtered = histories.filter((h) => {
        const referrer = h.referrerId as unknown as { username?: string; email?: string } | null;
        const referred = h.referredUserId as unknown as { username?: string; email?: string } | null;
        return (
          regex.test(referrer?.username || '') ||
    ...
    const data = filtered.map((h) => {
      const referrer = h.referrerId as unknown as { _id?: { toString(): string }; username?: string; email?: string };
      const referred = h.referredUserId as unknown as { _id?: { toString(): string }; username?: string; email?: string };
      return {
        _id: h._id.toString(),
