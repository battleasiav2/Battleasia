...
function resolveDemoBanner(map, fallbackImage) {
    if (PUBG_MAP_BANNERS.has(map))
        return getMapBannerPath(map);
    return fallbackImage || '';
}
function hoursFromNow(hours) {
    return new Date(Date.now() + hours * 3_600_000).toISOString();
}
async function ensureDemoUser(playerRole) {
...
async function createDemoMatchFromConfig(game, config, demoUser, fakePlayers) {
    ...
    for (let i = 0; i < participantUsers.length; i++) {
        ...
        if (demoUser && user._id.equals(demoUser._id) && !shouldJoinDemo)
            continue;
        ...
        if (config.completeWithResults) {
            results.push({
                ...
                avatar: user.avatar || '',
                status: (isWinner || i === 0 ? 'winner' : 'lose'),
                placement: i + 1,
                kills,
                points: kills * 3,
                winPrize,
...
async function seedDemoMatches(game, demoUser, fakePlayers) {
...
export async function seedPerGameDemoMatches(games) {
...
async function seedDemoHistory(demoUser, admin) {
    ...
    let balance = 0;
    const records = [
        { amount: 200, type: 'deposit', reason: 'deposit', note: 'bKash deposit' },
        { amount: 30, type: 'withdraw', reason: 'match_entry_fee', note: 'Miramar Squad Live' },
        { amount: 25, type: 'withdraw', reason: 'match_entry_fee', note: 'Sanhok Duo' },
        { amount: 85, type: 'deposit', reason: 'match_winnings', note: 'Sanhok Duo victory' },
        { amount: 15, type: 'withdraw', reason: 'match_entry_fee', note: 'Vikendi Solo' },
        { amount: 50, type: 'withdraw', reason: 'withdraw', note: 'Withdrawal request' },
    ];
...
async function seedDemoNotifications(demoUser, admin) {
...
async function seedDemoShopOrder(demoUser) {
...
async function seedDemoReferrals(demoUser, playerRole) {
...
async function seedDemoSupport(demoUser, admin) {
...
async function seedDemoPayments(demoUser) {
