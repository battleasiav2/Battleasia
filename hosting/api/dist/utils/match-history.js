...
export async function buildMyMatchHistory(userId) {
    ...
    return participations
        ...
        if (!match)
            return null;
        const resultEntry = (match.results || []).find((r) => r.participantId?.toString() === p._id.toString());
        const winnings = (Number(resultEntry?.winPrize) || 0) +
...
export async function buildUserMatchHistory(userId) {
    const participations = await MatchParticipant.find({ userId }).sort({ joinedAt: -1 });
    const matchIds = participations.map((p) => p.matchId);
