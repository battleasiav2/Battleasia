...
function toTopPlayer(row, extra) {
...
async function aggregatePlayerStats(sortField, limit = 5) {
...
async function buildMatchSummaries(matches) {
    if (!matches.length)
        return [];
    const matchIds = matches.map((m) => m._id);
    const participantCounts = await MatchParticipant.aggregate([
...
export async function getPublicDashboardStats() {
    const [winningsAgg, processedMatches, ongoingMatchesCount, topProfitPlayers, topPlayers, ongoingList, highPrizeList] = await Promise.all([
        Match.aggregate([
            { $match: { status: 'complete', 'results.0': { $exists: true } } },
            { $unwind: '$results' },
            {
