...
export async function enrichFeedsBatch(feeds, categoryMap, userId) {
    if (!feeds.length)
        return [];
    const authorIds = [
        ...new Set(feeds
    ...
    return feeds.map((feed) => {
        ...
        const authorRole = author?.role?.name
            ? {
...
export async function enrichFeed(feed, category, userId) {
    const categoryMap = new Map();
    if (category)
        categoryMap.set(feed.categoryId.toString(), category);
