...
router.post('/collection/start', requireAuth, async (req, res) => {
    try {
        const { amount, walletNumber, walletType } = req.body;
...
router.get('/collection/:merchantSerialNo/status', requireAuth, async (req, res) => {
    try {
...
router.get('/payout/:merchantSerialNo/status', requireAuth, async (req, res) => {
    try {
        const tx = await CoingoTransaction.findOne({
            merchantSerialNo: req.params.merchantSerialNo,
            userId: req.userId,
