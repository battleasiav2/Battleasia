...
const router = Router();
async function applyRoleToUser(user, roleId) {
    if (!roleId)
        return;
...
router.post('/', requireAuth, async (req, res) => {
    try {
        const { username, email, password, status = true, avatar = '', countryCode = '', mobileNo = '', pubgId = '', gameServer = '', referralCode, role, } = req.body;
...
router.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const { username, email, password, status, avatar, countryCode, mobileNo, pubgId, gameServer, referralCode, role, } = req.body;
        if (username)
        ...
        if (typeof pubgId === 'string')
            user.pubgId = pubgId;
        if (typeof gameServer === 'string')
