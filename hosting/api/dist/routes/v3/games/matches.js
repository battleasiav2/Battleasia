...
import { emitDashboardStatsUpdated, emitMatchCreated, emitMatchUpdated, } from '../../../utils/socket.js';
const router = Router();
...
router.get('/:id/participants', requireAuth, async (req, res) => {
    try {
        ...
        if (search)
            Object.assign(filter, buildSearchFilter(search, ['username', 'email', 'pubgId']));
...
router.put('/:id/results', requireAuth, async (req, res) => {
    try {
        ...
        if (Array.isArray(screenshots))
            match.resultScreenshots = screenshots;
...
router.post('/:id/distribute-winnings', requireAuth, async (req, res) => {
    try {
        ...
        for (const entry of match.results || []) {
            const totalWin = (Number(entry.winPrize) || 0) +
