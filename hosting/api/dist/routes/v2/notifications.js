...
router.patch('/:id/read', requireAuth, async (req, res) => {
    try {
        await NotificationRead.findOneAndUpdate({ userId: req.userId, notificationId: req.params.id }, { readAt: new Date() }, { upsert: true });
...
router.patch('/read-all', requireAuth, async (req, res) => {
    try {
        ...
        await Promise.all(notifications.map((n) => NotificationRead.findOneAndUpdate({ userId, notificationId: n._id }, { readAt: new Date() }, { upsert: true })));
