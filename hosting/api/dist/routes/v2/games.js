...
const router = Router();
async function getGameNameMap(gameIds) {
    const games = await Game.find({ _id: { $in: gameIds } });
    return new Map(games.map((g) => [g._id.toString(), g.name]));
}
async function enrichMatchesForUser(matches, userId) {
...
router.get('/matches/history/me', requireAuth, async (req, res) => {
    try {
...
router.post('/matches/:id/check-join', requireAuth, async (req, res) => {
    try {
        const match = await Match.findById(req.params.id);
        if (!match) {
            return res.status(404).json({ status: false, message: 'Match not found' });
        ...
        const issues = [];
        if (!user.pubgId)
            issues.push('PUBG ID is required');
        if (existing)
            issues.push('Already joined');
        if (match.status !== 'active' && match.status !== 'start')
            issues.push('Match is not joinable');
        if (participantCount >= match.totalPlayer)
            issues.push('Match is full');
        if (match.entryFee > (user.balance ?? 0) && !existing)
            issues.push('Insufficient balance');
        if (match.premiumOnly && user && !isUserPremium(user))
            issues.push('Premium required');
        return res.json({
            status: true,
            data: {
...
router.post('/matches/:id/join', requireAuth, async (req, res) => {
    try {
        const match = await Match.findById(req.params.id);
        if (!match) {
            return res.status(404).json({ status: false, message: 'Match not found' });
