...
async function saveVerificationCode(email, type) {
...
router.post('/signup', async (req, res) => {
    try {
        const { email, password, username, countryCode, mobileNo, pubgId, gameServer, referredBy, } = req.body;
...
router.get('/withdrawable-amount', requireAuth, async (req, res) => {
    try {
...
router.get('/referral-commissions', requireAuth, async (req, res) => {
    try {
        const { skip, limit } = parsePagination(req);
...
router.post('/premium/activate', requireAuth, async (req, res) => {
    try {
        ...
        const now = new Date();
        const base = user.premiumExpiresAt && user.premiumExpiresAt.getTime() > now.getTime()
...
async function listFollowUsers(field, userId, skip, limit) {
...
router.post('/:id/block', requireAuth, async (req, res) => {
    try {
        ...
        await UserBlock.findOneAndUpdate({ blockerId: req.userId, blockedId: targetId }, {}, { upsert: true });
