...
router.post('/reels', requireAuth, async (req, res) => {
    try {
        const user = await User.findById(req.userId);
        if (!user)
            return res.status(401).json({ status: false, message: 'Unauthorized' });
...
router.post('/reels/:id/view', requireAuth, async (req, res) => {
    try {
        const reel = await Reel.findByIdAndUpdate(req.params.id, { $inc: { totalViews: 1 } }, { new: true });
        if (!reel)
            return res.status(404).json({ status: false, message: 'Reel not found' });
    ...
    try {
        ...
        const otherIds = [
            ...new Set(conversations
                .map((conversation) => conversation.participants.find((participant) => participant.toString() !== userId)?.toString())
