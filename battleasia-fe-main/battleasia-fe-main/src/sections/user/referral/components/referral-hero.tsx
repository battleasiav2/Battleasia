...
export function ReferralHero({ title }: ReferralHeroProps) {
    ...
    >
      <Box
        sx={{
          position: 'absolute',
          inset: 0,
          backgroundImage: `url(${REFERRAL_HERO_IMAGE})`,
        ...
        <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap">
          ...
          <Box sx={{ ...getGlassBadgeChipSx(tokens), border: `1px solid ${alpha(USER_COLORS.gold, 0.35)}` }}>
            <Stack direction="row" alignItems="center" spacing={0.5} sx={{ px: 0.5 }}>
              <Iconify icon="solar:gift-bold" width={12} sx={{ color: USER_COLORS.gold }} />
              <Typography sx={{ fontSize: 10, fontWeight: 800, color: USER_COLORS.gold }}>{t('referral.badgeRewards')}</Typography>
            </Stack>
          </Box>
        </Stack>
      </Stack>
