...
export function OrdersHero({ title }: OrdersHeroProps) {
    ...
    >
      <Box
        sx={{
          position: 'absolute',
          inset: 0,
          backgroundImage: `url(${ORDERS_HERO_IMAGE})`,
        ...
        <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap">
          ...
          <Box sx={{ ...getGlassBadgeChipSx(tokens), border: `1px solid ${alpha(USER_COLORS.gold, 0.35)}` }}>
            <Stack direction="row" alignItems="center" spacing={0.5} sx={{ px: 0.5 }}>
              <Iconify icon="solar:bag-heart-bold" width={12} sx={{ color: USER_COLORS.gold }} />
              <Typography sx={{ fontSize: 10, fontWeight: 800, color: USER_COLORS.gold }}>{t('myOrders.badgeShop')}</Typography>
            </Stack>
          </Box>
        </Stack>
      </Stack>
