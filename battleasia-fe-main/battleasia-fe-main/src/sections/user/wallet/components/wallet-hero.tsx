...
export function WalletHero({ title }: WalletHeroProps) {
    ...
    >
      <Box
        sx={{
          position: 'absolute',
          inset: 0,
          backgroundImage: `url(${WALLET_HERO_IMAGE})`,
        ...
        <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap">
          ...
          <Box sx={{ ...getGlassBadgeChipSx(tokens), border: `1px solid ${alpha(USER_COLORS.gold, 0.35)}` }}>
            <Stack direction="row" alignItems="center" spacing={0.5} sx={{ px: 0.5 }}>
              <Iconify icon="solar:wallet-money-bold" width={12} sx={{ color: USER_COLORS.gold }} />
              <Typography sx={{ fontSize: 10, fontWeight: 800, color: USER_COLORS.gold }}>BAC</Typography>
            </Stack>
          </Box>
        </Stack>
      </Stack>
